...
public class Lab7 {
    ...
    private void generatePost(String[] row, Comment comment) {
        // TODO
        int postId = Integer.parseInt(row[1]);
        int usereId = Integer.parseInt(row[2]);
        //int commentingUserId = Integer.parseInt(row[4]);
        //DataStore.getInstance().getUsers().get(commentingUserId).getComments().add(comment);
        Map<Integer, Post> posts = DataStore.getInstance().getPosts();
        if (posts.containsKey(postId)) {
            posts.get(postId).getComments().add(comment);
        } else {
            Post post = new Post(postId, usereId);
    ...
    private void runAnalysis() {
        ...
        System.out.println();
        helper.getFiveInactiveUsersBasedOnPosts();
        System.out.println();
        helper.getFiveInactiveUsersBasedOnComments();
        System.out.println();
        helper.activeInactiveUsers();
        System.out.println();

        helper.userWithMoreLikes();
        helper.giveFiveMostLikedComment();
        helper.getPostWithhighestLikedComment();
    }
}
